...
export async function POST(request: Request) {
  try {
    ...
    const path = `${user.id}/builder/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${extension}`;
    ...
    return Response.json(
      { error: error instanceof Error ? error.message : "Unable to upload builder image." },
      { status: 500 }
    );
  }
}
